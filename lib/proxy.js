...
const {SYSTEM_PROXY_PORT, LISTEN_PORT} = process.env;
let PORT = LISTEN_PORT ? Number (LISTEN_PORT) : 5555; // default port if none on command line
const SYSTEM_PROXY = {
  ipaddress: "localhost", // Random public proxy
  port: SYSTEM_PROXY_PORT ? Number(SYSTEM_PROXY_PORT) : 1080,
...
function main(){
  ...
  for (let argn = 2; argn < process.argv.length; argn++) {
    ...
    if (process.argv[argn] === "--spp") {
      SYSTEM_PROXY.port = parseInt(process.argv[argn + 1]);
      argn++;
      continue;
    }
  }
  logger.fsock(`server listening on port ${PORT}`);
  // start HTTP server with custom request handler callback function
  const server = http.createServer(httpUserRequest).listen(PORT);
